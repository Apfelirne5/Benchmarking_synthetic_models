...
class _BaseTrainer:
    def __init__(
        ...
        (
            self.train_features,
            self.test_features,
        ) = utils.preprocess_features(train_features, test_features, args)
        self.train_labels, self.test_labels = train_labels, test_labels
        utils.print_feature_info("train", self.train_features, self.train_labels)
        utils.print_feature_info("test", self.test_features, self.test_labels)
    ...
    def __call__(self, trial: optuna.trial.Trial = None):
        ...
        if trial is not None:
            ...
            print("trial is None -> hps_dict: ", hps_dict)
...
def train(clf, dataloader, criterion, optimizer, epoch, device="cuda"):
    ...
    for i, (feature, label) in enumerate(dataloader):
        ...
        if not torch.isfinite(loss):
            print("==> Loss ({}) is not finite, terminating".format(loss.item()))
            optimizer.zero_grad()
            return -1
