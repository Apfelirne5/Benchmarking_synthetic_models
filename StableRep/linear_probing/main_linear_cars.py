...
def get_model_and_optimizer(args):
    ...
    print("model_param", count_parameters(model))
    model.cuda(args.gpu)
    if args.distributed:
...
def get_data_loaders(args):
    ...
    train_dataset = datasets.StanfordCars(args.data, split="train", transform=train_transform, download=False)
    val_dataset = datasets.StanfordCars(args.data, split="test", transform=val_transform, download=False)
...
def add_linear_classifier(feat_dim, num_classes, learning_rates, batch_size, use_bn=False):
    ...
    for blr in learning_rates:
        ...
        # print(count_parameters(linear_classifier))
