...
def visualize_dct(paths, names):
    ...
    for i, path in enumerate(paths):
        ...
        # Load the image using Pillow
            if j >0:
                break 
            img = Image.open(image_path).convert('RGB')  # Convert to grayscale after preprocessing 
            ...
            print(np.shape(dct_coefficients))
    ...
    # Show the plot
    output_path = f"DFT_Analysis_all_preprocessed.png"
    plt.savefig(output_path)

if __name__=="__main__":
    paths = [   "/visinf/projects_students/group8_dlcv/vilab08/PPCV_SD_Images/422x512/papilon/",
